...
export default async function ManageChaptersPage({ params }) {
  ...
  // Server Action สำหรับลบ Chapter และลบไฟล์รูปภาพในโฟลเดอร์
  async function deleteChapterAction(formData) {
    "use server";
    const chapterId = Number(formData.get("chapter_id"));

    try {
      // ดึงข้อมูลรูปภาพเพื่อไปลบไฟล์ออกจากเครื่อง
      ...
      for (const page of pages) {
        if (page.image_url.startsWith("/uploads/")) {
          ...
          await fs.unlink(filePath).catch(() => {}); // ลบไฟล์ ถ้าไม่พบให้ข้าม
        }
      }

      // ลบข้อมูลใน Database (Cascade จะลบ chapter_pages ให้อัตโนมัติ)
